                ...
                <div key={item.exercise} className="p-3 rounded-xl bg-white shadow overflow-x-auto">
                  ...
                  {item.first && item.latest ? (
                    <>
                      ...
                      {/* Weight change with arrows + highlight */}
                      <div className={`text-sm ${item.weightDiff>0?'text-green-600':item.weightDiff<0?'text-red-600':'text-gray-600'}`}>
                        Weight Change: {item.weightDiff}kg {item.weightDiff>0?'⬆️':item.weightDiff<0?'⬇️':''} ({item.pctWeight || 0}%)
                      </div>

                      {/* Reps change with arrows + highlight */}
                      <div className={`text-sm ${item.repsDiff>0?'text-green-600':item.repsDiff<0?'text-red-600':'text-gray-600'}`}>
                        Reps Change: {item.repsDiff} reps {item.repsDiff>0?'⬆️':item.repsDiff<0?'⬇️':''} ({item.pctReps || 0}%)
                      </div>
                    </>
                  ) : (
