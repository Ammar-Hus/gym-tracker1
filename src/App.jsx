...
export default function App(){
  ...
  function openDay(day){ setSelectedDay(day); setSelectedExercise(null); setSetsInput([{set:1,reps:8,weight:0}]); setTab('day'); }
  ...
  return (
    <div className={`min-h-screen ${primary} flex flex-col`}>
      ...
      {tab==='day' && (
        <div className="p-4">
          <button onClick={()=>setTab('dashboard')} className="text-blue-500 mb-3">← Back to Dashboard</button>
          {!selectedExercise && (
          ...
          {selectedExercise && (
            <div>
              <div className="font-semibold text-lg mb-2">{selectedExercise} - {selectedDay}</div>
              {setsInput.map((s,i)=>(
                <div key={i} className="flex space-x-2 mb-2">
                  <input type="number" className="border p-1 w-12" value={s.set} readOnly/>
                  <input type="number" className="border p-1 w-16" value={s.reps} onChange={e=>updateSet(i,'reps',e.target.value)}/>
                  <input type="number" className="border p-1 w-20" value={s.weight} onChange={e=>updateSet(i,'weight',e.target.value)}/>
                  <button className="text-red-500" onClick={()=>removeSet(i)}>X</button>
                </div>
              ))}
              <button className="bg-green-500 text-white px-3 py-1 rounded mr-2" onClick={addSetRow}>+ Set</button>
              <button className="bg-blue-500 text-white px-3 py-1 rounded" onClick={saveExercise}>Save</button>
            </div>
          )}
        </div>
      )}
      ...
      {tab==='progress' && (
        <div className="p-4 space-y-4">
          <div className="font-bold text-xl">Progress Summary</div>
          <div className="space-y-3">
            {progressSummary.map(p=>(
              <div key={p.exercise} className="bg-white p-3 rounded shadow">
                <div className="font-semibold">{p.exercise}</div>
                <div className="text-sm text-gray-600">First Entry: {p.first.weight} kg / {p.first.reps} reps</div>
                <div className="text-sm text-gray-600">Last 14 Days Avg: {p.avg14.avgWeight} kg / {p.avg14.avgReps} reps | +{p.pct14.weight??0}% weight, +{p.pct14.reps??0}% reps</div>
                <div className="text-sm text-gray-600">Monthly Avg: {p.avgMonth.avgWeight} kg / {p.avgMonth.avgReps} reps | +{p.pctMonth.weight??0}% weight, +{p.pctMonth.reps??0}% reps</div>
                <div style={{height:150}}>
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={chartDataForExercise(p.exercise)}>
